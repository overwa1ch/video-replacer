...
List references in the required `@图片1…N` order. Give each reference a distinct natural noun phrase that works in a fluent sentence. Each Job requirement must state the requested use of every bound `@图片N`; a bound file is not proof that the final prompt uses it. Include every Job exactly once; use an empty `references` array when it needs none.
...
Run `[LAUNCHER, "once"]`. Wait for the batch to appear at `needs-input/<batch>`, then verify that `batch-index.json` maps every expected source to the intended `V###` ID. Run `[LAUNCHER, "check", <batch>]` and resolve deterministic input errors before preparation. `check` builds a provisional reference-index view in memory; it does not freeze or write `reference-index.json`. `prepare` performs that one-time freeze. Once a flow or streaming result exists, a missing or changed index fails closed instead of being regenerated.
...
The deterministic parent reads each active source video with trusted local FFmpeg and creates a bounded chronological, uniform-timestamp frame set beginning at the opening frame. Supported backend sources are at most 30 seconds, for which the current cadence is 0.75 seconds; the bounded policy spreads samples across an unexpectedly longer input. Source evidence is cached only when source size/SHA, the sampler recipe and verified FFmpeg identity all match. Before flow freeze, `source-evidence-index.json` anchors every cache manifest, frame digest, sampler recipe and FFmpeg identity; its SHA-256 is part of the flow fingerprint. Every Job receives independent ordinary-file copies and revalidates their hashes; references, requirements, Codex output, prompt Gate, probe and plan remain per Job. The isolated Codex turn receives only those attached frames, the ordered bound reference images and structured Job JSON; the source video itself is not attached. With `CODEX_EXEC_SERVER_URL=none`, the turn has no execution/filesystem tools, returns the prompt in its structured result, and cannot write a file. It uses the same setup-managed stable external, instruction-free, strict-file-auth `CODEX_HOME` whose zero-tool request and authenticated local handshake Doctor attested; this home has no `AGENTS.md`, rules, skills, plugins, MCP configuration or hooks. On Windows the parent repeats official package/release/digest verification and validates the fixed-drive/private-DACL home boundary immediately before executing Codex. The JavaScript scheduler starts prompt preparations in stable `V###` FIFO order with effective concurrency 1; the auth lock remains a safety fallback. Queue time, auth-lock wait and Codex execution time are recorded separately. Before writing `prompt.txt`, the parent replaces the node's binding line with the declared ordered binding, resolves `@视频N` and `@图片N` handles inside the exact Job requirement wording, inserts that wording as a fixed highest-priority block, then adds a separate model-generated execution section. The final Gate requires the entire binding and requirement prefix to match parent declarations, every bound semantic name to appear in the model section, and submission-plan images to equal the declared references in order. Shot-heading prose findings remain advisory; they do not override explicit user wording. No source/reference analysis artifact is handed to another model node, and no stage scores, approves, rejects or reviews the reference images.

`prepare` keeps `PAUSE`, performs no backend upload, creates no remote task, and returns every eligible Job as locally prepared or blocked. The batch workflow state is `LOCAL_PREPARED_AWAITING_APPROVAL` only when no eligible Job is blocked; a partial result remains `LOCAL_PREPARATION_BLOCKED` and does not request payment approval. `READY_FOR_SUBMISSION` remains the internal per-Job technical status and does not itself grant payment authority. The workflow writes local upload-preparation evidence before probe and the submission plan.

If one local preparation result is `BLOCKED/FAILED`, or a READY result has a submission plan that fails deterministic revalidation, run `[LAUNCHER, "retry-prepare", <batch>, <V###>]`. This requires `needs-input/` plus `PAUSE`, archives the old result while keeping the canonical result intact until atomic replacement, rejects any Job with submission/checkpoint/task evidence, and reruns only that local Job. It never creates a payment checkpoint or calls a paid command.
...
Before creating any payment checkpoint, the control plane validates every Job's frozen preparation result and submission plan as a batch. An old prompt-pipeline flow may resume only when it is fully prepared and its prompt already passes the current canonical binding, immutable-requirement, model-section, and ordered-image checks; otherwise rebuild it as a new schema-v3 batch. Copy the previous `requirements.txt` and unchanged bindings byte-for-byte into that rebuild. Change only a user-authorized semantic field, and never paraphrase a hard constraint to make a node or Gate pass.

If the session is lost, inspect `status <batch> --json`, `loop-state.json`, and the external request ledger. Resume a recorded task ID. An uncertain submission remains blocked until the ledger establishes a safe recovery path.
