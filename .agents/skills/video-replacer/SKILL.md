...
3. Build a new clean batch with copied regular files, `requirements.txt`, schema-v3 `job-bindings.json`, and `PAUSE`. Preserve the user's hard constraints verbatim; every bound reference must have a clear requested use in that Job's requirement line.
...
5. Run `once` until the batch is indexed under `needs-input/`, run the read-only `check <batch>`, then run `prepare <batch>` and wait on that same process.
6. For local preparation, verify `status <batch>` reports the batch workflow state, report it, and stop with `PAUSE` present. Use `retry-prepare <batch> <V###>` only for one recorded `BLOCKED/FAILED` local preparation Job, or a READY Job whose local submission plan fails deterministic revalidation. It archives the prior attempt transactionally and never authorizes payment.
...
- `check` validates a not-yet-frozen reference index in memory and does not write it. `prepare` freezes that index once; a missing or changed index after a flow/result exists is a blocker, not a rebuild request.
...
- Leave prompt-node construction to the deterministic parent. It samples the source locally with trusted FFmpeg, attaches only timestamped frames, ordered references and structured Job JSON, and starts one Codex turn with `CODEX_EXEC_SERVER_URL=none`; that turn has no execution/filesystem tools or discovery and returns only a structured prompt string. The parent replaces the node binding with the declared ordered binding, inserts the Job requirements as a fixed highest-priority block, separates the model-generated execution section, requires every bound semantic name in that model section, and alone writes `prompt.txt`.
...
- Require Doctor's two local zero-tool wire phases before READY. Under one stable auth lock, both use the production zero-tool request surface, same home, synthetic image and loopback Responses provider. The `requires_openai_auth=false` capture is required check `codex-wire`; it adds one final `cli_auth_credentials_store="ephemeral"` isolation override and requires empty top-level/additional tools, no multi-agent hints, an image, and no auth or account-routing header. The `requires_openai_auth=true` capture is required check `codex-file-auth-wire`; it uses the production file-auth command, requires the same surface, and proves its Bearer matches strictly validated stable `auth.json` by in-memory SHA-256 plus `hmac.compare_digest`. Raw token/header values are not retained or logged. Each captured model request must route to loopback and stop at HTTP 418 before a model response or inference. Any managed/system MCP, hook, tool or agent-hint reinjection is a setup failure. This evidence does not attest unrelated process network activity; the phases use synthetic media and contain no video-backend or paid command.
- The JavaScript scheduler runs eligible prompt preparations in stable `V###` FIFO order with effective concurrency 1. The auth lock remains a safety fallback, not a second scheduler. Queue, auth-lock and Codex execution durations are recorded separately.
- Jobs with the same verified source SHA, sampler recipe and FFmpeg identity share only the deterministic sampled-frame cache. Each Job receives independent ordinary frame copies and still runs its own reference staging, requirements, prompt turn, Gate, probe and submission plan.
...
- Treat old prompt-pipeline state as recovery evidence. Rebuild a partial or unknown flow as a new schema-v3 batch, copying `requirements.txt` and each unchanged binding byte-for-byte. Never paraphrase or weaken a user requirement to make a node or Gate pass.
